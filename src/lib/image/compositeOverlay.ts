...
export async function withWallpaperOverlay(
  ...
  const overlay = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">${buildGalaxyMarkup(width, height, astrology.generationDate)}${buildPlanetDiagramsMarkup(astrology, width, height, flushPlanets)}${buildDateHeaderMarkup(width, height, astrology.generationDate, reading.intent)}${buildLuckMeterMarkup(width, height, reading.luckScore)}</svg>`;
